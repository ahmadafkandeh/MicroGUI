...
void LCD_SetPointColor(unsigned short color)
{
	POINT_COLOR=color;
}
void LCD_DrawCharTransparent (unsigned int x, unsigned int y, unsigned short *c) 
{
  int idx = 0, i, j;
  x = WIDTH-x-CHAR_W;
...
{       
  ...
  unsigned int    i, j;
  LCD_WindowMax(xsta,ysta,xsta+xend,ysta+yend);
	...
	if(!mode) 											/*非叠加方式		   */
	{
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {                 
		        if(temp&0x01)
				{
       				LCD_DrawPoint(x+t,y+pos,TextColor);
				}
// 				else
// 				{
...
// 				}
	...
	}else												/*叠加方式   */
	{
		for(pos=0;pos<size;pos++)
		{
			...
			for(t=0;t<size/2;t++)
		    {                 
		        if(temp&0x01)LCD_DrawPoint(x+t,y+pos,TextColor);	/*画一个点*/     
		        temp>>=1; 
		    }
		}
	}	    
...
*********************************************************************************************************
*/	
void LCD_PutString(u8 x,u16 y,const char *p)
{         
    while(*p!='\0')
    {       
...
void LCD_PutStringInBox(u8 x,u16 y,char *p,unsigned int Start,unsigned int Len)
{  
	if (Start<=strlen(p))
	{
		p=(char*)((int)p+Start);
...
unsigned short GetDevCode(void)
{
	return DeviceCode;
}
